...
It's been a while, so I don't remember all my problems with the engine, but I do remember that the way the engine was structured tied entities directly to sprites, as in sprites _were_ entities with hitboxes and state machines. The sprite entity amalgamate was also limited to one state machine, which I had to hack my way around in order to implement the player controller correctly. The engine was weirdly opinionated. It was hardcoded to use _Tiled_, and maps had to be tilemaps.

Also, the game _Have a Nice Death_ came out a year we thought of the idea, and we had no idea the game existed.
